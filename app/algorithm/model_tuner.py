...
def tune_hyperparameters(
    ...
    def objective(hyperparameters):
        ...
        valid_X, valid_y = valid_data["X"].astype(np.float), valid_data["y"].astype(
            np.float
        )
        ...
        # train model
        model, _ = model_trainer.train_model(
            train_X, train_y, valid_X, valid_y, hyperparameters
        )
        ...
        if np.isnan(opt_metric) or math.isinf(opt_metric):
